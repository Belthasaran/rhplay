...
function linuxRetroarchAppImageCandidates(programDataDir) {
  if (!programDataDir) return [];
  return [
    path.join(programDataDir, 'RetroArch-Linux-x86_64.AppImage'),
    path.join(programDataDir, 'RetroArch-Linux-x86_64', 'RetroArch-Linux-x86_64.AppImage'),
  ];
}

function appImageHomeCoreDir(appImagePath) {
  if (!appImagePath) return '';
  return path.join(
    path.dirname(appImagePath),
    `${path.basename(appImagePath)}.home`,
    '.config',
    'retroarch',
    'cores'
  );
...
function getBundledRetroarchAppImagePath(programDataDir) {
  const candidates = linuxRetroarchAppImageCandidates(programDataDir);
...
function retroarchExeCandidates(programDataDir = '') {
  ...
  if (process.platform === 'linux' && programDataDir) {
    bundled.push(...linuxRetroarchAppImageCandidates(programDataDir));
  }
...
function retroarchCoreCandidates(retroarchExe, programDataDir = '') {
  ...
  if (programDataDir) {
    ...
    if (process.platform === 'linux') {
      for (const appImage of linuxRetroarchAppImageCandidates(programDataDir)) {
        const coreDir = appImageHomeCoreDir(appImage);
        if (coreDir) bases.push(coreDir);
      }
    }
  }
  ...
  if (retroarchExe) {
    ...
    if (process.platform === 'linux' && retroarchExe.endsWith('.AppImage')) {
      const coreDir = appImageHomeCoreDir(retroarchExe);
      if (coreDir) bases.push(coreDir);
    }
  }
