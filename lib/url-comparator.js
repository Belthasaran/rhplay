...
class UrlComparator {
  ...
  isSignificantUrlChange(oldUrl, newUrl, oldSize, newSize) {
    // URL addition (we didn't have a URL before) - this is a minor change, not major
    // It doesn't require a download since we're just adding metadata
    if (!oldUrl && newUrl) {
      return { significant: false, reason: 'url_added', details: { newUrl } };
    }
    
    // URL removal (we had a URL but now it's gone) - this is significant
    if (oldUrl && !newUrl) {
      return { significant: true, reason: 'url_removed', details: { oldUrl } };
    }
    
    // Both missing - no change
    if (!oldUrl && !newUrl) {
      return { significant: false, reason: 'both_missing' };
    }
