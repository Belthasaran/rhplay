...
class BinaryFinder {
  ...
  /**
   * Get platform-specific binary names for flips
  ...
  getFlipsNames() {
    if (this.isWindows) {
      ...
      return ['flips-macos', 'flips'];
    } else if (this.isLinux) {
      return ['flips-linux', 'flips'];
    } else {
      return ['flips'];
    }
  }
  ...
  /**
   * Get platform-specific binary names for asar
   * Returns array with platform-specific name first, then generic name
   */
  getAsarNames() {
    if (this.isWindows) {
      ...
      return ['asar-macos', 'asar'];
    } else if (this.isLinux) {
      return ['asar-linux', 'asar'];
    } else {
      return ['asar'];
    }
  }
   ...
   */
  findFlips() {
    const executableNames = this.getFlipsNames();

    const checks = [
      { name: 'Database setting 1', fn: () => this.checkDatabase('flipsPath') },
      { name: 'Database setting 2', fn: () => this.checkDatabase('flips_path') },
      { name: 'Environment variable', fn: () => this.checkEnvironment('FLIPS_BIN_PATH') },
      { name: 'Current directory', fn: () => this.checkCurrentDirectoryWithPlatformNames(executableNames) },
      { name: 'Script directory', fn: () => this.checkScriptDirectoryWithPlatformNames(executableNames) },
      { name: 'Common directories', fn: () => this.checkCommonDirectoriesWithPlatformNames(executableNames) },
      { name: 'System PATH', fn: () => this.checkSystemPathWithPlatformNames(executableNames) },
    ];
   ...
   */
  findAsar() {
    const executableNames = this.getAsarNames();
    
    const checks = [
      { name: 'Database setting 1', fn: () => this.checkDatabase('asarPath') },
      { name: 'Database setting 2', fn: () => this.checkDatabase('asar_path') },
      { name: 'Environment variable', fn: () => this.checkEnvironment('ASAR_BIN_PATH') },
      { name: 'Current directory', fn: () => this.checkCurrentDirectoryWithPlatformNames(executableNames) },
      { name: 'Script directory', fn: () => this.checkScriptDirectoryWithPlatformNames(executableNames) },
      { name: 'Asar-specific directories', fn: () => this.checkAsarDirectoriesWithPlatformNames(executableNames) },
      { name: 'Common directories', fn: () => this.checkCommonDirectoriesWithPlatformNames(executableNames) },
      { name: 'System PATH', fn: () => this.checkSystemPathWithPlatformNames(executableNames) },
    ];
  ...
  findSmwRom() {
    const checks = [
      { name: 'Database setting 1', fn: () => this.checkDatabase('romPath') },
      { name: 'Database setting 2', fn: () => this.checkDatabase('vanillaRomPath') },
      { name: 'Database setting 3', fn: () => this.checkDatabase('sfc_path') },
      ...
      { name: 'Common directories', fn: () => this.checkCommonDirectories('smw.sfc') },
  ...
  /**
   * Check current directory with platform-specific names (tries platform name first)
   */
  checkCurrentDirectoryWithPlatformNames(executableNames) {
    for (const name of executableNames) {
      const result = this.checkCurrentDirectory(name);
      if (result) {
        return result;
      }
    }
  ...
  /**
   * Check script directory with platform-specific names (tries platform name first)
   */
  checkScriptDirectoryWithPlatformNames(executableNames) {
    for (const name of executableNames) {
      const result = this.checkScriptDirectory(name);
      if (result) {
        return result;
      }
    }
  ...
  /**
   * Check asar-specific directories with platform-specific names (tries platform name first)
   */
  checkAsarDirectoriesWithPlatformNames(executableNames) {
    // Try platform-specific names in each directory
    const basePaths = [
      path.join(this.projectRoot, 'bin'),
      this.projectRoot,
      '/mnt/c/snesgaming/bin',
      '/usr/local/bin',
    ];

    for (const basePath of basePaths) {
      for (const name of executableNames) {
        const checkPath = path.join(basePath, name);
        if (fs.existsSync(checkPath)) {
          return checkPath;
        }
      }
    ...
    // Also check for asar.exe on Windows
    if (this.isWindows) {
      const exePath = path.join(this.projectRoot, 'asar.exe');
  ...
  checkRomDirectories() {
    ...
    // If clientDbPath is provided (e.g., from Electron app), also check userData directory
    if (this.clientDbPath && fs.existsSync(this.clientDbPath)) {
      const userDataDir = path.dirname(this.clientDbPath);
      romPaths.push(path.join(userDataDir, 'smw.sfc'));
      romPaths.push(path.join(userDataDir, 'SuperMarioWorld.sfc'));
    }
  ...
  checkCommonDirectories(filename) {
	...
	path.join(appData, 'rhtools', filename),
	path.join(appData, 'rhplay', filename),
  ...
  /**
   * Check common directories with platform-specific names (tries platform name first)
   */
  checkCommonDirectoriesWithPlatformNames(executableNames) {
    for (const name of executableNames) {
      const result = this.checkCommonDirectories(name);
      if (result) {
        return result;
      }
    }
  ...
  /**
   * Check system PATH with platform-specific names (tries platform name first)
   */
  checkSystemPathWithPlatformNames(executableNames) {
    for (const name of executableNames) {
      const result = this.checkSystemPath(name);
      if (result) {
        return result;
      }
    }
