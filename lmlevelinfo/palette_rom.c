...
static PaletteSource palette_from_custom(const LevelInfo *info, uint8_t pal256[256][3],
  ...
  // Mirror rows 8-15 only when the custom blob left them identical to rows 0-7 (sprite rows often differ).
  for (int pal_line = 8; pal_line < 16; pal_line++) {
    int need_mirror = 1;
    for (int c = 0; c < 16 && need_mirror; c++) {
      int src = (pal_line & 7) * 16 + c;
      int dst = pal_line * 16 + c;
      if (pal256[dst][0] != pal256[src][0] || pal256[dst][1] != pal256[src][1] ||
          pal256[dst][2] != pal256[src][2]) {
        need_mirror = 0;
      }
    }
    if (!need_mirror) continue;
    for (int c = 0; c < 16; c++) {
      int src = (pal_line & 7) * 16 + c;
      int dst = pal_line * 16 + c;
