...
const { decodeBlob: decodePatchBlob } = require('./patchblob-decode');
...
class RecordCreator {
  ...
  createGameVersionRecord(gameid, metadata, primaryPatch, primaryBlobData, resourceTracking = null) {
    ...
    // Extract sa1 from multiple sources in JSON
    // Priority: 1) tags array contains "sa1" -> "yes"
    //           2) fields.sa1 as "Yes"/"No" string -> "yes"/"no"
    //           3) raw_fields.sa1 as true/false boolean -> "yes"/"no"
    let sa1 = null;
    if (Array.isArray(metadata.tags) && metadata.tags.includes('sa1')) {
      sa1 = 'yes';
    } else if (metadata.fields && metadata.fields.sa1 !== undefined && metadata.fields.sa1 !== null) {
      const sa1Value = String(metadata.fields.sa1).trim();
      if (sa1Value.toLowerCase() === 'yes') {
        sa1 = 'yes';
      } else if (sa1Value.toLowerCase() === 'no') {
        sa1 = 'no';
      }
    } else if (metadata.raw_fields && metadata.raw_fields.sa1 !== undefined && metadata.raw_fields.sa1 !== null) {
      if (metadata.raw_fields.sa1 === true || metadata.raw_fields.sa1 === 'true') {
        sa1 = 'yes';
      } else if (metadata.raw_fields.sa1 === false || metadata.raw_fields.sa1 === 'false') {
        sa1 = 'no';
      }
    }
  ...
  async updatePatchBlobRecord(pbuuid, gvuuid, gameid, patchFile, blobData) {
    ...
    const patchblobData = {
      ...
      result_sha256: patchFile.result_sha256 || null,
  ...
  async decodeBlob(encryptedData, keyBase64) {
    return decodePatchBlob(encryptedData, keyBase64);
  }
