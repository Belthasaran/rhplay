...
static const uint32_t kVanillaPtrTableBases[] = {
    0x00BE80u, /* vanilla SMW ROM pointer init */
    0x009326u, /* akogare-style LM hack (PC 0x1326) */
};
...
int map16_rom_get_vanilla_tile(const Rom *rom, uint16_t tile_id, Map16Tile *out) {
  ...
  /* LM hacks often store Map16 in banks 0x18/0x19; try those before vanilla 0x0D. */
  static const uint8_t kBanks[] = {0x18u, 0x19u, 0x0Du};
  for (size_t bi = 0; bi < sizeof(kBanks) / sizeof(kBanks[0]); bi++) {
    if (read_block_at_snes_ptr(rom, ptr16, kBanks[bi], out)) return 1;
  }
  return 0;
}
