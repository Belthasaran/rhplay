...
- Added `--package` / `--import` support to `jstools/newgame.js`, enabling authors to create shareable `.rhpak` bundles (skeleton JSON, prepared blobs, encrypted resources/screenshots) and rehydrate them on another system prior to database insertion.
