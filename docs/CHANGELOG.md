- **Provisioner AppImage module path**: `prepare_databases.js` resolves `lib/provision-bundle` and related modules from `app.asar` when run from `app.asar.unpacked`, fixing Refresh Plan failures on packaged Linux AppImage builds.
- **Provisioner platform appfiles**: `prepare_databases.js` resolves `base:linux64` / `base:win64` / `base:macos` for appfiles targets (e.g. `retroarch_deploy.txt`); provision errors now fail the run and surface in the Provisioner UI instead of reporting success.
- **patchblob1_sha224 fix**: `newgame.js` now writes encoded outer blob SHA-224 to `patchblobs.patchblob1_sha224` (not decoded `pat_sha224`); added `jstools/fix-patchblob1-sha224.js` repair script; `result_sha256` populated on prepare/upsert; hash field audit aligned with `updategames.js` export path. Findings + repair how-to: `docs/NEWGAME_PATCHBLOB1_SHA224_FINDINGS.md`.
- **Manifest migrate_artifacts**: `dbmanifest.json` `base`/`sqlpatches` entries may set `"migrate_artifacts": true` to move blob columns into `{userData}/artifacts/` during provisioning.
- **Provision conflict resolution**: Bundle patches include `patch_declarations`; pre-patch reconcile uninstalls overlapping RHPAKs, skips feed-equivalent rows, remaps clientdata via `gv_migrations.csv`, orphan cleanup. See `devdocs/PROVISION_CONFLICTS.md`.
- **Manifest chain shortcut**: Light provisioning inherits full-chain `base`/`sqlpatches` when `base:light` is not configured (both `file_name` and `sha256` required); divergent targets with configured `base:light` use independent light patches only.
- **find_waiting_notincluded.js --latest-waiting-csv**: Export latest waiting row per group as waiting_index CSV to stdout or `--latest-waiting-csv=PATH`; skips groups whose newest member is already in the database.
- **find_waiting_notincluded.js cluster cache**: Writes `waiting_cache_clusters.json` beside the index CSV for incremental fuzzy-match grouping on subsequent runs (`--rebuild-cache`, `--no-cache`, `--cluster-cache=PATH`).
- **find_waiting_notincluded.js**: New CLI to find waiting-index CSV entries not in gameversions, group related submissions via BPS hash and fuzzy name/author matching, with `--older-than`, `--hidematches-all`, `--hidematches-oldonly`, and `--json` options.
- **Installed RHPAKs Show System filter**: Manage > Show Installed RHPAKS modal adds unchecked-by-default "Show System" checkbox to hide `is_system=1` RHPAKs; GUI uninstall skips system RHPAKs; `rhpak:uninstall` blocks system RHPAKs unless `allowSystemUninstall: true` is explicitly passed.
- **updategames RHPAK conflicts**: Added `rhpakuuid2` multi-owner tracking on RHPAK-owned tables, `rhpaks.is_system`, `updategames_conflicts.js` review/edit tool, and `lib/rhpak-ownership.js` helpers.
- **updategames RHPAK conflicts plan**: Added `devdocs/UPDATEGAMES_RHPAK_CONFLICTS_PLAN.md` — documents SMWC vs RHPAK ownership conflicts on in-place import, and plans `updategames_conflicts.js` (review + interactive `--edit-ownership`) plus `rhpakuuid2` multi-owner and `rhpaks.is_system` schema extensions.
- **ASAR Ignore Warnings extrapatch flag**: Edit System Patch Definitions adds an ASAR-only "Ignore Warnings" checkbox; when set, +Patch/staging treats ASAR stderr warnings as non-fatal (fail only on non-zero exit or unchanged ROM).
- **Stage automated testing**: RetroArch + SNI stage auto-test framework (`lib/stage-autotest/`) with 7-phase runner (boot, navigate, verify `$13BF`, flags, fail/retry, structured logs); Edit Stages **Auto Test** button, CLI `jstools/stage_autotest.js`, config at `{userData}/stage-autotest/tester_config.json`. USB2SNES library/address from app Settings; SNI connects after emulator launch with retry. See `docs/STAGE_AUTOTEST.md`.
- **Cancel Run re-prepare**: Standard Cancel Run keeps Prepare Run open, converts expanded rows back to a concrete plan (random entries become fixed game/stage rows), and Stage and Save creates a new run; optional `cancelledFromRunUuid` stored in `config_json`.
- **Run launch cur_booted unmask**: Launch paths reveal masked random entries (with DB fallback) before writing `cur_booted.json`/`cur_booted.html`, so auto-advance no longer records `(random)`/`???`.
...
- **Run types and Free Play mode**: Prepare Run toolbar relabels, match-count boxes in Game/Stage Limits dropdowns, Type selector (Standard Run / Free Play). Schema adds `runs.run_type` and optional `prerequisites_json` on plan/result entries; export/import/runinfo include run type. Free Play runs start with no active stage, use per-row Play buttons, and Finalize Run instead of Cancel.
...
- **gameversions27zfolder.js**: Retrospective catalog backfill from `gameversions` DB rows using local ZIP search (`--sourcezips-folder`) with download fallback; same catalog layout as updategames export; optional `--screenshot-db` reads verified screenshots from `screenshot.db` before HTTP download.
- **Catalog image extensions**: Catalog export resolves screenshot filenames with extensions from URL, screenshot.db `file_ext`, or magic-byte detection; extensionless fallback when type is unknown.
- **gameversions27zfolder exit fix**: Script exits cleanly after Summary; catalog export kills timed-out lmfilter child processes so they do not keep Electron alive.
- **Catalog screenshot.db order fix**: `downloadGameImages` tries screenshot.db before HTTP for all image URLs (including those with `.png` in the URL), not only extensionless URLs.
- **Run staging pre-launch monitoring**: Starting a run after Game 1 launch from the Run Staged modal now attaches the same exit/ROM monitor as the active-run Launch button (emulator sessionId and USB2SNES basename polling).
...
- **Emulator single-instance launch**: New launches stop any tracked instance of the configured emulator first (1s wait, then relaunch); also detects and stops the same emulator binary if already running outside RHPlay (e.g. after app restart); Quick Launch shows **Running — RESTART** instead of disabling the button; SNI mode on port 23074 triggers USB2SNES reconnect ~1s after emulator launch.
...
- **SMWResource social merge + attestation**: Refresh merges local/hosted social IDs (verified/addedAt/attestation recency) and PUTs merged list; verifies signed `/profile/me` responses; auto-trusts RHServer notary at Operating Admin on verified connect; `canUserAttest` trust permission; npub default subject fingerprint in trust wizard; copy icons for trust declaration keys. RHServer operator docs: `rhserver/docs/NOTARY_SETUP.md`, `rhserver/docs/SERVER_SECRETS.md`.
...
- **Launcher build (Windows)**: `launcher:install-app-deps` no longer uses `cd rhtools-launcher && ../node_modules/.bin/electron-builder` (fails on Windows `cmd.exe` with `'..' is not recognized`). It runs [`scripts/launcher-install-app-deps.cjs`](scripts/launcher-install-app-deps.cjs) so `install-app-deps` executes with `cwd` set to `rhtools-launcher` via Node.

- **RHTools Launcher — slim AppImage/portable**: The launcher now installs **runtime-only** dependencies under `rhtools-launcher/package.json` (IPFS/Arweave fetch, `ethers`, `dns-query`, `better-sqlite3`, `lzma-native`, `tar`, etc.) instead of copying the entire repository `node_modules`. Build scripts run `npm install --prefix rhtools-launcher`, `launcher:install-app-deps` (native rebuild), `launcher:prune-for-pack` (`npm prune --omit=dev`), then `electron-builder`. `electron-builder.cjs` no longer includes a manual `node_modules` FileSet (electron-builder bundles production deps automatically; duplicating caused EINST/EEXIST). `lib/` in the pack is limited to `binary-finder.js` (SMW ROM check). Example: extracted `app.asar` `node_modules` drops from ~1.2GB (full monorepo tree) to ~235MB (closure).
