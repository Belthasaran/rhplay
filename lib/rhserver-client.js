...
class RHServerClient {
  constructor(options = {}) {
    ...
    this.onVerifiedResponse = options.onVerifiedResponse || null;
  }
  ...
  async submitStageFeedbackBulk(items) {
    const res = await this.apiRequest('POST', '/stage_feedback/bulk', { items });
    if (!res.ok) throw new Error(`stage_feedback/bulk failed: HTTP ${res.status}`);
    return res.json();
  }

  async getStageFeedbackStatus(since) {
    const q = typeof since === 'number' && Number.isFinite(since) ? `?since=${since}` : '';
    const res = await this.apiRequest('GET', `/stage_feedback/status${q}`);
    if (!res.ok) throw new Error(`stage_feedback/status failed: HTTP ${res.status}`);
    return res.json();
  }
  ...
  _verifyResponseJson(json) {
    if (!this.verifyResponses || !json?.server_signature) {
      return json;
    }
    const result = verifyServerSignature(json, json.server_signature);
    if (!result.ok) {
      throw new Error(`RHServer response signature invalid: ${result.reason}`);
    }
    if (typeof this.onVerifiedResponse === 'function') {
      this.onVerifiedResponse(json);
    }
    return json;
  }
  ...
  async getProfileMe() {
    ...
    return this._verifyResponseJson(json);
  }

  async updateProfileMe(payload) {
    const res = await this.apiRequest('PUT', '/profile/me', payload);
    if (!res.ok) throw new Error(`profile/me PUT failed: HTTP ${res.status}`);
    const json = await res.json();
    return this._verifyResponseJson(json);
  }
}
