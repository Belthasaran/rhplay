...
async function downloadGameImages(game, gameid, imagesDir, logFn = console.log, options = {}) {
  ...
  for (const imageUrl of images) {
    ...
    if (path.extname(urlBase)) {
      if (dryRun) {
        logFn(`    [DRY RUN] Would download ${urlBase}`);
        continue;
      }
      const result = await downloadImage(imageUrl, lastImageRequestTime);
      lastImageRequestTime = Date.now();
      if (result.success && result.buffer) {
        writeCatalogImageAtomic(gameImagesDir, urlBase, result.buffer);
        logFn(`    Catalog: downloaded ${urlBase}`);
        screenshotFiles.push(urlBase);
      } else {
    ...
    if (screenshotDb && screenshotDataDir) {
      if (dryRun) {
        logFn(`    [DRY RUN] Would try screenshot.db for ${urlBase} (detect extension)`);
        continue;
      }
    ...
    if (dryRun) {
      logFn(`    [DRY RUN] Would download ${urlBase} (detect extension from bytes)`);
      continue;
    }
