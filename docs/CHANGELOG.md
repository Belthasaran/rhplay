- **patchblob1_sha224 fix**: `newgame.js` now writes encoded outer blob SHA-224 to `patchblobs.patchblob1_sha224` (not decoded `pat_sha224`); added `jstools/fix-patchblob1-sha224.js` repair script; `result_sha256` populated on prepare/upsert; hash field audit aligned with `updategames.js` export path.
