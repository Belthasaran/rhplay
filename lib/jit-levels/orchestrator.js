...
function getDbDetectedLevels(db, gameid, version) {
  ...
  const levelnames = db.prepare(`
    ...
    FROM levelnames ln
    JOIN gameversion_levelnames gvl ON ln.lnuuid = gvl.lnuuid
...
async function runJitLevelDetection(params) {
  ...
  for (const list of [namesResult.levels, transResult.levels, lmfilterLevels]) {
  ...
  const dbLevels = includeDbSources ? getDbDetectedLevels(db, gameid, version).levels : [];
