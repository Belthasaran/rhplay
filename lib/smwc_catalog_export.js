...
const { tryLoadScreenshotFromDb } = require('./screenshot_db_reader');
...
/**
 * Build catalog metadata from a gameversions DB row (gvjsondata + authoritative columns).
 */
function metadataFromGameversionRow(row) {
  let base = {};
  if (row.gvjsondata) {
    try {
      base = typeof row.gvjsondata === 'string' ? JSON.parse(row.gvjsondata) : { ...row.gvjsondata };
    } catch (e) {
      base = {};
    }
  }
  ...
  const merged = {
    ...base,
    gameid,
    id: base.id != null ? base.id : gameid,
    section: row.section != null ? row.section : base.section,
    name: row.name != null ? row.name : base.name,
    time: row.time != null ? row.time : base.time,
    added: row.added != null ? row.added : base.added,
    moderated: row.moderated != null ? row.moderated : base.moderated,
    author: row.author != null ? row.author : base.author,
    authors: row.authors != null ? row.authors : base.authors,
    submitter: row.submitter != null ? row.submitter : base.submitter,
    demo: row.demo != null ? row.demo : base.demo,
    featured: row.featured != null ? row.featured : base.featured,
    length: row.length != null ? row.length : base.length,
    difficulty: row.difficulty != null ? row.difficulty : base.difficulty,
    url: row.url != null ? row.url : base.url,
    download_url: row.download_url != null ? row.download_url : base.download_url,
    name_href: row.name_href != null ? row.name_href : base.name_href,
    author_href: row.author_href != null ? row.author_href : base.author_href,
    obsoleted_by: row.obsoleted_by != null ? row.obsoleted_by : base.obsoleted_by,
    size: row.size != null ? row.size : base.size,
    description: row.description != null ? row.description : base.description,
    tags: row.tags != null ? row.tags : base.tags,
    tags_href: row.tags_href != null ? row.tags_href : base.tags_href,
    fields_type: row.fields_type != null ? row.fields_type : base.fields_type,
    legacy_type: row.legacy_type != null ? row.legacy_type : base.legacy_type,
    raw_difficulty: row.raw_difficulty != null ? row.raw_difficulty : base.raw_difficulty,
    combinedtype: row.combinedtype != null ? row.combinedtype : base.combinedtype,
    gametype: row.gametype != null ? row.gametype : base.gametype,
    local_resource_filename: row.local_resource_filename != null
      ? row.local_resource_filename
      : base.local_resource_filename
  };

  if (typeof merged.tags === 'string') {
    try {
      merged.tags = JSON.parse(merged.tags);
    } catch (e) {
      merged.tags = merged.tags ? [merged.tags] : [];
    }
  }
  if (!Array.isArray(merged.tags)) {
    merged.tags = merged.tags ? [merged.tags] : (base.tags || []);
  }
  if (!Array.isArray(merged.images)) {
    merged.images = base.images || [];
  }

  if (row.download_url && !merged.original_download_filename) {
    merged.original_download_filename = row.download_url.split('/').pop();
  }
  if (base.original_download_filename) {
    merged.original_download_filename = base.original_download_filename;
  }

  if (base.authors_json) merged.authors_json = base.authors_json;
  if (base.submitter_json) merged.submitter_json = base.submitter_json;
  if (base.raw_fields) merged.raw_fields = base.raw_fields;
  if (base.fields) merged.fields = base.fields;
  if (base.warnings != null) merged.warnings = base.warnings;
...
const KNOWN_IMAGE_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.gif', '.webp'];

function urlBasename(imageUrl) {
...
function normalizeFileExt(fileExt) {
  if (!fileExt) return null;
  const ext = String(fileExt).trim().toLowerCase();
  if (!ext) return null;
...
function resolveCatalogImageFilename(imageUrl, opts = {}) {
  const { fileExt, buffer } = opts;
  ...
  const hintExt = normalizeFileExt(fileExt);
  if (hintExt) {
    return `${basename}${hintExt}`;
  }

  if (buffer) {
    const detected = detectImageExtFromBuffer(buffer);
    if (detected) {
...
function findExistingCatalogImagePath(gameImagesDir, basename) {
  if (!basename || !gameImagesDir) return null;

  const exactPath = path.join(gameImagesDir, basename);
  if (fs.existsSync(exactPath) && fs.statSync(exactPath).isFile()) {
    return { path: exactPath, filename: basename };
  }

  if (!path.extname(basename)) {
    for (const ext of KNOWN_IMAGE_EXTENSIONS) {
      const withExt = `${basename}${ext}`;
      const candidate = path.join(gameImagesDir, withExt);
      if (fs.existsSync(candidate) && fs.statSync(candidate).isFile()) {
        return { path: candidate, filename: withExt };
      }
    }
  }
...
function writeCatalogImageAtomic(gameImagesDir, finalFilename, buffer) {
  const imagePath = path.join(gameImagesDir, finalFilename);
...
function maybeRenameLegacyExtensionlessFile(existing, finalFilename, gameImagesDir) {
  if (existing.filename === finalFilename) {
    return existing;
  }
  const finalPath = path.join(gameImagesDir, finalFilename);
  if (fs.existsSync(finalPath)) {
    return { path: finalPath, filename: finalFilename };
  }
  fs.renameSync(existing.path, finalPath);
  return { path: finalPath, filename: finalFilename };
}

function normalizeExistingScreenshotFilename(gameImagesDir, filename) {
  if (path.extname(filename)) return filename;

  const existing = findExistingCatalogImagePath(gameImagesDir, filename);
  if (!existing) return filename;

  try {
    const header = Buffer.alloc(12);
    const fd = fs.openSync(existing.path, 'r');
    fs.readSync(fd, header, 0, 12, 0);
    fs.closeSync(fd);
    const detected = detectImageExtFromBuffer(header);
    if (!detected) return existing.filename;

    const finalFilename = `${filename}${detected}`;
    const renamed = maybeRenameLegacyExtensionlessFile(existing, finalFilename, gameImagesDir);
...
async function downloadImage(imageUrl, lastRequestTime) {
  ...
  try {
    ...
    return { success: true, buffer: imageData };
  } catch (error) {
    return { success: false, error: error.message };
  }
}

async function downloadGameImages(game, gameid, imagesDir, logFn = console.log, options = {}) {
  ...
  const { screenshotDb, screenshotDataDir, dryRun = false } = options;
  const actionLabel = screenshotDb ? 'loading' : 'downloading';
  logFn(`  Catalog: ${actionLabel} ${images.length} image(s)...`);
  let lastImageRequestTime = 0;
  for (const imageUrl of images) {
    if (!imageUrl || typeof imageUrl !== 'string') continue;
    const urlBase = urlBasename(imageUrl);
    if (!urlBase) continue;

    const existing = findExistingCatalogImagePath(gameImagesDir, urlBase);
    if (existing) {
      let finalFilename = existing.filename;
      if (!path.extname(urlBase)) {
        try {
          const buffer = fs.readFileSync(existing.path);
          const resolved = resolveCatalogImageFilename(imageUrl, { buffer });
          if (resolved !== existing.filename) {
            const renamed = maybeRenameLegacyExtensionlessFile(existing, resolved, gameImagesDir);
            finalFilename = renamed.filename;
          }
        } catch (e) {
          // keep existing filename
        }
      }
      screenshotFiles.push(finalFilename);
      continue;
    }

    if (screenshotDb && screenshotDataDir) {
      if (dryRun) {
        logFn(`    [DRY RUN] Would try screenshot.db for ${urlBase}`);
        continue;
      }
      const dbResult = tryLoadScreenshotFromDb(screenshotDb, gameid, imageUrl, screenshotDataDir);
      if (dbResult.ok && dbResult.buffer) {
        const finalFilename = resolveCatalogImageFilename(imageUrl, {
          fileExt: dbResult.fileExt,
          buffer: dbResult.buffer
        });
        writeCatalogImageAtomic(gameImagesDir, finalFilename, dbResult.buffer);
        logFn(`    Catalog: loaded from screenshot.db (${dbResult.source}): ${finalFilename}`);
        screenshotFiles.push(finalFilename);
        continue;
      }
      if (dbResult.reason) {
        logFn(`    Catalog: screenshot.db miss for ${urlBase}: ${dbResult.reason}`);
      }
    }

    if (dryRun) {
      const extNote = path.extname(urlBase) ? '' : ' (detect extension from bytes)';
      logFn(`    [DRY RUN] Would download ${urlBase}${extNote}`);
      continue;
    }

    const result = await downloadImage(imageUrl, lastImageRequestTime);
    lastImageRequestTime = Date.now();
    if (result.success && result.buffer) {
      const finalFilename = resolveCatalogImageFilename(imageUrl, { buffer: result.buffer });
      writeCatalogImageAtomic(gameImagesDir, finalFilename, result.buffer);
      logFn(`    Catalog: downloaded ${finalFilename}`);
      screenshotFiles.push(finalFilename);
    } else {
      logFn(`    Catalog: image download failed ${urlBase}: ${result.error}`);
    }
  }
...
function getExistingScreenshotFiles(gameid, imagesDir) {
  ...
  return fs.readdirSync(gameImagesDir)
    ...
    .map(f => normalizeExistingScreenshotFilename(gameImagesDir, f))
    .sort();
}
...
function spawnWithTimeout(command, args, options = {}, timeoutMs = 20000) {
  return new Promise((resolve, reject) => {
    ...
    timeoutId = setTimeout(() => {
      ...
      finish(reject, new Error(`Process ${command} exceeded timeout of ${timeoutMs}ms`));
    }, timeoutMs);
    child.on('exit', (code, signal) => {
      ...
      if (signal) finish(reject, new Error(`Process ${command} killed by signal ${signal}`));
      else finish(resolve, { status: code, stdout, stderr });
    });
    child.on('error', (error) => {
      if (!killed) finish(reject, error);
    });
  });
}
...
async function runLmFilter(resultSfcPath, resultSha1, tempDir) {
  try {
    ...
    if (result.status !== 0) {
	    console.log(`lmfilter failed ${result.status}`)
	   return null;
    }
    ...
    try {
      ...
      if (!bpsAlreadyExists) {
        ...
        translevelData = runFindTranslevels(tempResultPath, resultHash, tempDir);
	      ...
	      console.log("bpsAlreadyExists=true")
      }
...
module.exports = {
  ...
  getSmwcSectionKey,
  metadataFromGameversionRow,
  normalizeMetadata,
  detectImageExtFromBuffer,
  resolveCatalogImageFilename,
  findExistingCatalogImagePath,
  urlBasename,
  downloadGameImages,
