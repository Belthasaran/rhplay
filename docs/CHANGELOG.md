- **Packaging fix**: Include `lib/jit-levels/` and `lib/profile-connect-params.js` in electron-builder `files` (fixes startup crash: Cannot find module `../lib/jit-levels/orchestrator`).
