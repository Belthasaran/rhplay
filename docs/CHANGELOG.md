- **DB provisioner light chain (Phase 6)**: `prepare_databases.js --db-chain full|light` and `--lightdb`; provisioned.json `chain` field (`full` explicit, `light`, or omitted for legacy implicit full); chain-aware updates in `database-update-check.js`; Provisioner + launcher UI checkbox (light default) refreshes plan on toggle.
