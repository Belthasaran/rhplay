...
async function tryServerUrls(ctx, patchblob, options) {
  let client = ctx.rhserverClient;
  if (typeof ctx.getRhserverClient === 'function') {
    client = await ctx.getRhserverClient();
  }
...
async function methodFromArtifactStore(ctx, patchblob, options, wantEncoded) {
  const userDataDir = ctx && ctx.userDataPath ? ctx.userDataPath : null;
  const resolved = artifactStore.resolvePblobPath({
    pbuuid: patchblob.pbuuid,
    userDataDir
  });
  if (!resolved) {
    const attachment = getAttachment(ctx, patchblob.patchblob1_name);
    if (attachment && attachment.auuid) {
      const byAuuid = artifactStore.resolvePblobPath({ auuid: attachment.auuid, userDataDir });
      if (byAuuid) {
        return methodFromArtifactStoreFile(ctx, patchblob, byAuuid, options, wantEncoded);
      }
    }
    return null;
  }
  return methodFromArtifactStoreFile(ctx, patchblob, resolved, options, wantEncoded);
}

async function methodFromArtifactStoreFile(ctx, patchblob, resolved, options, wantEncoded) {
  emitProgress(options, { phase: 'resolve', method: 0, message: 'Loading patch from artifact store...' });
  const data = fs.readFileSync(resolved.filePath);
  const attachment = getAttachment(ctx, patchblob.patchblob1_name) || {};
  const blobCheck = verifyPatchblobBuffer(data, patchblob, {
    ...attachment,
    file_hash_sha224: attachment.file_hash_sha224,
    file_hash_sha256: resolved.row.file_hash_sha256 || attachment.file_hash_sha256
  });
  if (!blobCheck.ok) return null;
  if (wantEncoded) {
    return { data, path: resolved.filePath };
  }
  if (!patchblob.patchblob1_key) {
    throw new Error(`No decryption key for ${patchblob.patchblob1_name}`);
  }
  const decoded = await decodeBlob(data, patchblob.patchblob1_key);
  const check = verifyPatchBuffer(decoded, patchblob, attachment);
  if (!check.ok) return null;
  return { data: decoded, path: resolved.filePath, verified: check.verified };
}
...
async function runPatchblobMethods(ctx, patchblob, gameVersion, options) {
  const attempts = [];
  const chain = [
    () => methodFromArtifactStore(ctx, patchblob, options, true),
...
async function runPatchMethods(ctx, patchblob, gameVersion, options) {
  ...
  let resolved = await tryMethod(0, () => methodFromArtifactStore(ctx, patchblob, options, false));
  if (resolved) return resolved;

  resolved = await tryMethod(1, () => methodFromDatabase(ctx, patchblob, options, false));
  if (resolved) return resolved;
