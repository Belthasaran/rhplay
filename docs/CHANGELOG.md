...
- **Manifest migrate_artifacts**: `dbmanifest.json` `base`/`sqlpatches` entries may set `"migrate_artifacts": true` to move `patchbin`/`resource`/`screenshot` blob columns into `{userData}/artifacts/` during provisioning (`lib/migrate-artifacts.js`). rhserver light-base packaging now VACUUM/checkpoints stripped DBs before `base:light` xz artifacts.
