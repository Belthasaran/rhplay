...
function parseProvindexDoc(parsed) {
  ...
  if (parsed && typeof parsed === 'object') {
    return {
      db_file: parsed.db_file || null,
      version_before: parsed.version_before ?? null,
      version_after: parsed.version_after ?? null,
      chain: parsed.chain || null,
      patch_declarations: parsed.patch_declarations || {},
      instructions: Array.isArray(parsed.instructions) ? parsed.instructions : []
    };
  }
  throw new Error(`${PROVINDEX_FILENAME} must be a JSON array or object`);
}
...
function loadProvindex(bundleDir) {
  ...
  return parseProvindexDoc(parsed);
}

async function loadProvindexFromArchive(archivePath, format, stagingDir = null) {
  const bundleDir = stagingDir || fs.mkdtempSync(path.join(os.tmpdir(), 'provindex-read-'));
  const owned = !stagingDir;
  try {
    await unpackBundle(archivePath, format, bundleDir);
    const payloadDir = fs.existsSync(path.join(bundleDir, 'payload'))
      ? path.join(bundleDir, 'payload')
      : bundleDir;
    return { ...loadProvindex(payloadDir), bundleDir: payloadDir };
  } finally {
    if (owned && stagingDir == null) {
...
async function applyBundle({
  ...
  try {
    ...
    const payloadDir = fs.existsSync(path.join(bundleDir, 'payload'))
      ? path.join(bundleDir, 'payload')
      : bundleDir;
    const provindex = loadProvindex(payloadDir);

    const hasExtractDb = provindex.instructions.some((i) => String(i.type || '').toUpperCase() === 'EXTRACT_DB');
    if (!hasExtractDb && dbPath && extractFile) {
      const member = resolveBundleMember(payloadDir, extractFile);
      fs.mkdirSync(path.dirname(dbPath), { recursive: true });
      fs.copyFileSync(member, dbPath);
    ...
    await executeProvindex({
      instructions: provindex.instructions,
      bundleDir: payloadDir,
      dbPath,
      dbName,
      userDataDir,
      onLog
    });
    return { bundleDir: payloadDir, dbPath, provindex };
  } finally {
