...
const CARL_ADDRESSES = {
  ...
  // NOTE: $7F8000-$7F8182 is SMW's sprite routine - DO NOT USE!
  CARL_INITIALIZED_FLAG: 0x7F8190,   // Per-module init flags (moved past SMW routine)
  CARL_MODULE_TABLE: 0x7F81C0,       // Module address table
  CARL_HOOK_ADDR: 0x7F8200,          // Hook caller code
  
  // Frame hook address (trampoline)
  FRAME_HOOK_RAM: 0x7F8300           // Trampoline code (JSL CARL_HOOK, execute overwritten instr, RTL)
};
...
class CarlModuleLoader {
  ...
  async installFrameHook() {
    ...
    console.log(`[CarlModuleLoader] Original code at $${SMW_RAM_ROUTINE.toString(16).toUpperCase()}: ${Array.from(originalCode).map(b => '$' + b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`);
    
    // Verify SMW has initialized the routine (should be A9 F0 8D 01)
    if (originalCode[0] === 0x00 && originalCode[1] === 0x00) {
      console.error('[CarlModuleLoader] ERROR: $7F8000 not initialized! SMW hasn\'t created the sprite routine yet.');
      console.error('[CarlModuleLoader] Wait for SMW to fully boot, then try !load again.');
      return {
        success: false,
        message: '✗ SMW RAM routine not initialized yet - wait for game to boot fully'
      };
    }
    
    // Check if already patched (would start with $22 = JSL)
    if (originalCode[0] === 0x22) {
      console.log('[CarlModuleLoader] Detected existing JSL at $7F8000 - verifying integrity...');
      
      // Verify the JSL target is our trampoline
      const existingTarget = originalCode[1] | (originalCode[2] << 8) | (originalCode[3] << 16);
      const expectedTarget = CARL_ADDRESSES.FRAME_HOOK_RAM;
      
      if (existingTarget === expectedTarget) {
        console.log('[CarlModuleLoader] ✓ Hook intact and pointing to correct trampoline');
        
        // Verify trampoline is still there
        const trampolineProtocolAddr = 0xF50000 + (expectedTarget - 0x7E0000);
        const trampolineCheck = await this.snes.GetAddresses([[trampolineProtocolAddr, 4]]);
        const trampolineStart = trampolineCheck[0];
        
        if (trampolineStart[0] === 0x22) { // Should start with JSL to CARL_HOOK_ADDR
          console.log('[CarlModuleLoader] ✓ Trampoline intact - hook is fully functional');
          return {
            success: true,
            message: '✓ Frame hook already installed and verified',
            data: {
          ...
          console.warn('[CarlModuleLoader] ⚠ Trampoline corrupted! Reinstalling...');
          // Fall through to reinstall
        }
      } else {
        console.warn(`[CarlModuleLoader] ⚠ JSL points to wrong address: $${existingTarget.toString(16).toUpperCase()}`);
        console.warn(`[CarlModuleLoader] Expected: $${expectedTarget.toString(16).toUpperCase()}`);
        console.warn(`[CarlModuleLoader] Hook may be from previous session or corrupted. Reinstalling...`);
        // Fall through to reinstall
      }
    }
    
    // Verify expected bytes (A9 F0 = LDA #$F0)
    // If we get here, bytes are not a valid hook, so they should be the original routine
    if (originalCode[0] !== 0xA9 || originalCode[1] !== 0xF0) {
      console.warn(`[CarlModuleLoader] WARNING: Unexpected bytes at $7F8000`);
      console.warn(`[CarlModuleLoader] Expected original: $A9 $F0 (LDA #$F0)`);
      console.warn(`[CarlModuleLoader] Found: $${originalCode[0].toString(16).toUpperCase()} $${originalCode[1].toString(16).toUpperCase()}`);
      console.warn(`[CarlModuleLoader] This may be corrupted state from soft reset.`);
      console.warn(`[CarlModuleLoader] Will attempt to patch anyway...`);
      
      // Pause extra long to let SMW finish initialization
      console.log(`[CarlModuleLoader] Waiting 500ms for SMW to stabilize after soft reset...`);
    ...
    // Ensure 8-bit accumulator BEFORE executing the overwritten instruction
    // (CARL modules or previous code might have left it in 16-bit mode)
    trampoline.push(0xE2);  // SEP #$20 (Set 8-bit A)
    trampoline.push(0x20);
    ...
    // Problem: JSL pushed $7F8004 on stack, but that's the middle of the STA instruction!
    // Original bytes were 5 bytes total: A9 F0 (LDA) + 8D 01 02 (STA) = 5 bytes
    // We need to continue at $7F8005 (next complete instruction)
    // Solution: Adjust return address on stack from $7F8004 to $7F8005
    
    // Pull return address low byte, increment it, push it back
    // (A is already 8-bit from SEP #$20 above)
    trampoline.push(0x68);  // PLA (pull low byte = $04)
    trampoline.push(0x1A);  // INC A (increment to $05)
    trampoline.push(0x48);  // PHA (push $05 back)
    
    // Now RTL will pop the adjusted address ($7F8005)
    ...
    console.log(`  2. SEP #$20 → Ensure 8-bit accumulator (CARL module may have changed it)`);
    console.log(`  3. LDA #$F0, STA $0201 → Execute overwritten instruction (init OAM sprite Y pos)`);
    console.log(`  4. PLA, INC A, PHA → Adjust return address $7F8004 → $7F8005 (skip incomplete byte)`);
    console.log(`  5. RTL → Return to $7F8005 (next complete instruction)`);
    ...
    // Pause the game to prevent race condition during patch
    // $13D4 is SMW's pause flag (0=running, 1=paused)
    const SMW_PAUSE_FLAG = 0x7E13D4;
    const pauseFlagProtocolAddr = 0xF50000 + (SMW_PAUSE_FLAG - 0x7E0000);
    
    // Read current pause state
    const pauseStateArray = await this.snes.GetAddresses([[pauseFlagProtocolAddr, 1]]);
    const wasPaused = pauseStateArray[0][0];
    
    console.log(`[CarlModuleLoader] Pausing game to safely install hook...`);
    await this.snes.PutAddress([[pauseFlagProtocolAddr, Buffer.from([1])]]);
    
    // Wait for pause to take effect
    await new Promise(resolve => setTimeout(resolve, 100));
    
    // Patch $7F8000 with JSL to our trampoline
    // CRITICAL: This must be atomic! SMW calls this routine many times per frame.
    // We pause the game to prevent race conditions during the write.
    ...
    // Try up to 3 times to patch (in case of race condition)
    let patchSuccess = false;
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`[CarlModuleLoader] Patching attempt ${attempt}/3...`);
      ...
      const verifyArray = await this.snes.GetAddresses([[hookProtocolAddr, 4]]);
      const verifyBytes = verifyArray[0];
      
      if (Buffer.compare(hookPatch, verifyBytes) === 0) {
        console.log(`[CarlModuleLoader] ✓ Patch verified on attempt ${attempt}`);
        patchSuccess = true;
        break;
      } else {
        console.warn(`[CarlModuleLoader] ⚠ Patch verification failed on attempt ${attempt}`);
        console.warn(`[CarlModuleLoader]   Expected: ${Array.from(hookPatch).map(b => '$' + b.toString(16).toUpperCase().padStart(2, '0')).join(' ')}`);
        console.warn(`[CarlModuleLoader]   Got:      ${Array.from(verifyBytes).map(b => '$' + b.toString(16).toUpperCase().padStart(2, '0')).join(' ')}`);
        
        if (attempt < 3) {
          console.log(`[CarlModuleLoader] Retrying...`);
          await new Promise(resolve => setTimeout(resolve, 100));
        }
      }
    }
    ...
    console.log(`[CarlModuleLoader] Restoring game state (was ${wasPaused ? 'paused' : 'running'})...`);
    await this.snes.PutAddress([[pauseFlagProtocolAddr, Buffer.from([wasPaused])]]);
    
    if (!patchSuccess) {
      console.error('[CarlModuleLoader] ✗ Failed to install hook after 3 attempts');
      console.error('[CarlModuleLoader] This should be extremely rare now that we pause the game!');
      return {
        success: false,
        message: '✗ Failed to patch $7F8000 after 3 attempts - unexpected error'
      };
    }
    
    console.log(`[CarlModuleLoader] ✓ Patched $${SMW_RAM_ROUTINE.toString(16).toUpperCase()} with JSL to trampoline`);
    console.log(`[CarlModuleLoader] ✓ Return address adjusted: $7F8004 → $7F8005 (skip incomplete instruction)`);
    console.log(`[CarlModuleLoader] Stack flow: ROM → [ROM_addr] → JSL adds [$7F8004] → Trampoline adjusts to [$7F8005] → RTL → $7F8005 → Original RTL → ROM`);
...
`;
  ...
  /**
   * Clear the frame hook and restore original SMW routine
   * Useful after soft resets when hook may be corrupted
   * 
   * @returns {Promise<Object>} Result
   */
  async clearFrameHook() {
    console.log('[CarlModuleLoader] Clearing frame hook and restoring original routine...');
    
    const SMW_RAM_ROUTINE = 0x7F8000;
    const hookProtocolAddr = 0xF50000 + (SMW_RAM_ROUTINE - 0x7E0000);
    
    // Restore original bytes: A9 F0 8D 01 (LDA #$F0, STA $0201)
    const originalBytes = Buffer.from([0xA9, 0xF0, 0x8D, 0x01]);
    await this.snes.PutAddress([[hookProtocolAddr, originalBytes]]);
    
    console.log('[CarlModuleLoader] ✓ Restored original routine at $7F8000');
    console.log('[CarlModuleLoader] Note: Modules are still loaded. Use !unloadall to clear modules.');
    console.log('[CarlModuleLoader] You can now use !load to reinstall the hook cleanly.');
    
    return {
      success: true,
      message: '✓ Frame hook cleared - original routine restored'
    };
  }
}
