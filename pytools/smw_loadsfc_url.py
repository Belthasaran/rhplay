...
def repatch_loadsfc_url_function(args,ccrom=False,noexit=False):
    hackinfo = {'id' : '0'}
    ...
    if re.search(r'https?:', bpsurl):
        ...
        dldata = req.content
    elif re.match(r'[.a-zA-Z_0-9-]+\.(zip|sfc)$',bpsurl, re.I):
        if os.path.exists(bpsurl):
            f_handle = open(bpsurl, 'rb')
        elif os.path.exists(os.path.join(path_prefix, bpsurl)):
            f_handle = open(os.path.join(path_prefix, bpsurl), 'rb')
        elif 'SFCFOLDER' in os.environ and os.environ['SFCFOLDER']:
            for foldern in os.environ['SFCFOLDER'].split(':'):
               if os.path.exists(os.path.join(foldern, bpsurl)):
                   f_handle = open(os.path.join(foldern, bpsurl), 'rb')
                   break
        dldata = f_handle.read()
        f_handle.close()
    else:
        dldata = None
        raise ValueError(f'Invalid filename [{bpsurl}]')
    ...
    f1.write(dldata)
    f1.close()
    os.replace(os.path.join(path_prefix, "temp", xsha224) + ".new", os.path.join(path_prefix, "rom", shake1))
    hackinfo["patch"] = ""
    hackinfo["rom"] = os.path.join(path_prefix, "rom", shake1)
    ...
    if ccrom:
        namesuffix='.cc'
        #
        #infile = os.path.join(path_prefix,'temp', 'result')
        #infile_s = os.path.join(path_prefix, "temp",  xsha224)
        infile_s = os.path.join(path_prefix, "rom", shake1)
        outfile_s = os.path.join(path_prefix,'temp', 'result2')
        print("CMD:" + flips_cmd+" --apply " + os.path.join(path_prefix,'zips', 'ccSuperMarioWorld.ips') +"  "+infile_s+" " +outfile_s)
        os.system(flips_cmd+" --apply " + os.path.join(path_prefix,'zips', 'ccSuperMarioWorld.ips') +"  "+infile_s+" " +outfile_s)
        #resultfile=os.path.join('temp','result2')
        resultfile = outfile_s
        resultf = open(resultfile, "rb")
        data = resultf.read()
        resultf.close()
        #shake1_patched = (base64.b64encode(hashlib.shake_128(data).digest(24), b"_-")).decode('latin1')
    ...
    if re.match('sfcurl', args[0]) or re.match('.*repatch.*', args[0]):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 1:
        ...
        sys.argv[0]='repatch'
    rp_result = repatch_loadsfc_url_function(sys.argv,ccrom=ccrom)
