...
async function downloadGameImages(game, gameid, imagesDir, logFn = console.log, options = {}) {
  ...
  for (const imageUrl of images) {
    ...
    if (screenshotDb && screenshotDataDir) {
      if (dryRun) {
        logFn(`    [DRY RUN] Would try screenshot.db for ${filename}`);
        continue;
      }
      const dbResult = tryLoadScreenshotFromDb(screenshotDb, gameid, imageUrl, screenshotDataDir);
      if (dbResult.ok && dbResult.buffer) {
        const tempPath = `${imagePath}.tmp`;
        fs.writeFileSync(tempPath, dbResult.buffer);
        fs.renameSync(tempPath, imagePath);
        logFn(`    Catalog: loaded from screenshot.db (${dbResult.source}): ${filename}`);
        screenshotFiles.push(filename);
        continue;
      }
      if (dbResult.reason) {
        logFn(`    Catalog: screenshot.db miss for ${filename}: ${dbResult.reason}`);
      }
    }
